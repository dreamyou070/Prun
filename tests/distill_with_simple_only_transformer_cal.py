...
class TemporalWindowAttention(nn.Module):
    def __init__(self, dim, layer_name, heads=8):
    ...
    def forward(self,
        ...
        hidden_states = self.proj_in(hidden_states)  # [batch*pixel, num_frames, dim]

        # [2] 슬라이딩 윈도우 분할
        self.window_size = int(num_frames / 2)
        ...
        attn_output = (attn_output[None, None, :].reshape(batch_size, height, width, num_frames, channel).permute(0, 3, 4, 1,2).contiguous())
        attn_output = attn_output.reshape(batch_frames, channel, height, width)
        ...
        for name, subnet in net.named_children():
            ...
            if subnet.__class__.__name__ == 'TransformerTemporalModel' or subnet.__class__.__name__ == 'AnimateDiffTransformer3D':
                if final_name in skip_layers :
                    ...
                    simple_block = TemporalWindowAttention(basic_dim, layer_name=final_name)
...
def main(args) :
    ...
    teacher_pipe = AnimateDiffPipeline.from_pretrained(args.pretrained_teacher_model,
                                                       motion_adapter=teacher_adapter, torch_dtype=weight_dtype)
    ...
    student_pipe = AnimateDiffPipeline.from_pretrained(args.pretrained_teacher_model,
                                                       motion_adapter=student_adapter,torch_dtype=weight_dtype)
    ...
    def register_hook(unet, hook_fn):
        ...
        def hooking(net):
            for name, subnet in net.named_children():
                ...
                if subnet.__class__.__name__ == 'TemporalWindowAttention':
                    subnet.register_forward_hook(hook_fn)
