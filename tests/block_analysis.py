...
def main(args):
    ...
    #for name, module in pruned_unet.named_modules():
        # check all modules
    #    print(f' name = {name}, module = {module.__class__.__name__}')

    logger.info(f'\n step 8. Block Caching (Reuse)')

    for p, prompt in enumerate(custom_prompts):
        if p < 100 :  # 0,1,2
            save_folder = os.path.join(student_out_dir, f'sample_{str(p)}')
