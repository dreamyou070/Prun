...
def main(args):
    ...
    print(f' step 2. get model')
    print(f' (2.1) teacher model')
    weight_dtype = torch.float32
    teacher_adapter = MotionAdapter.from_pretrained("wangfuyun/AnimateLCM", torch_dtype=weight_dtype)
    teacher_pipe = AnimateDiffPipeline.from_pretrained(args.pretrained_teacher_model,
    ...
    print(f' (2.2) create online unet ')
    # test _unet
    test_adapter = MotionAdapter.from_pretrained("wangfuyun/AnimateLCM", torch_dtype=weight_dtype)
    test_pipe = AnimateDiffPipeline.from_pretrained(args.pretrained_teacher_model, motion_adapter=test_adapter,
                                                    torch_dtype=weight_dtype)
    test_pipe.load_lora_weights("wangfuyun/AnimateLCM",
                                weight_name="AnimateLCM_sd15_t2v_lora.safetensors",
                                adapter_name="lcm-lora")
    ...
    # make controller
    motion_editor = MotionControl(is_teacher=False,
                                        train=True,
                                        frame_num=16,
                                        skip_layers=pruned_blocks,)
    register_motion_editor(test_unet, motion_editor)
