...
def main(args):
    ...
    #args.csv_path = r'/scratch2/dreamyou070/Prun/result/teacher_calibration_data/sample.csv'
    #args.video_folder = r'/scratch2/dreamyou070/Prun/result/teacher_calibration_data/sample'
    ...
    block_list = ['down_blocks.0.motion_modules.0','down_blocks.0.motion_modules.1','down_blocks.1.motion_modules.0','down_blocks.1.motion_modules.1',
                  'down_blocks.2.motion_modules.0','down_blocks.2.motion_modules.1','down_blocks.3.motion_modules.0','down_blocks.3.motion_modules.1',
    ...
    for name, param in pruned_unet.named_parameters():
        # set pruned layer to zero
        if 'motion' in name:
    ...
    for block_idx, pruned_layer in enumerate(block_list):
        # [0] check original layer
        for layer_name in state_dict.keys():
            if pruned_layer in layer_name:
                print(f'pruning layer : {layer_name}')
                state_dict[layer_name] = torch.zeros_like(state_dict[layer_name])

        # [1] make pruned unet
        pruned_unet.load_state_dict(state_dict)
        # [2] make_folder
        pruned_layer_folder = os.path.join(student_out_dir, f'block_{pruned_layer}')
            ...
            if step < 100 :
                ...
                with torch.no_grad():
                    ...
                    student_pred_x_0 = get_predicted_original_sample(student_output,
                                                                     ...
                                                                     alpha_schedule,
                                                                     sigma_schedule)
                    # make video from student_pred_x_0
                    # gradient detach
                    decode_chunk_size = 16
                    student_video_tensor = test_pipe.decode_latents(student_pred_x_0.detach(), decode_chunk_size)
                    student_video = test_pipe.video_processor.postprocess_video(video=student_video_tensor, output_type='pil')[0] # list of 16 pillow
        ...
        # [4] evaluation
        save_pruned_layer_folder = os.path.join(student_out_dir, f'save_block_{pruned_layer}')
        os.makedirs(save_pruned_layer_folder, exist_ok=True)
        for i, prompt in enumerate(custom_prompts):
            if i < 100 :
                evaluation(prompt, test_pipe, None, save_pruned_layer_folder, n_prompt, num_frames, guidance_scale,
                           num_inference_steps, h=args.h, weight_dtype=weight_dtype)
                if block_idx == 0 :
                    # do also teacher
                    evaluation(prompt, teacher_pipe, None, teacher_out_dir, n_prompt, num_frames, guidance_scale,
                               num_inference_steps, h=args.h, weight_dtype=weight_dtype)
