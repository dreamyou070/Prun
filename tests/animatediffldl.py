...
class GeneticAlgorithm:
    ...
    def model_inference(self, architecture, is_teacher=False, ):
        ...
        if is_teacher:
            ...
            for layer_name in test_state_dict.keys(): # state dictionary
...
def main(args) :
    ...
    print(f' step 3. algorithmic search')
    #for k in unet.state_dict().keys():
    #    print(f'state dict key name = {k}')

    print(f' step 4. algorithmic search')
    init_sd = [0,1,2,3,4,5,9,10,11,12,13,14,15]
    ...
    ga = GeneticAlgorithm(total_block_num=len(total_control_blocks),
                     select_num=10,
                     population_size=50,
                     mutation_num=9,
                     crossover_num=10,
