...
class SimpleAttention(nn.Module):
    ...
    def __init__(self, dim, layer_name=""):
        ...
        output_channels = dim
        kernel_size = 3
        self.conv1d = nn.Conv1d(input_channels, output_channels, kernel_size, padding=kernel_size // 2)
        self.relu = nn.ReLU()
    ...
    def forward(self,
        ...
        x = self.relu(x)
        x = self.pool(x)
        return TransformerTemporalModelOutput(sample=x)
class DDIMSolver:
