...
def main(args):
    ...
    if args.report_to == 'wandb':
        import wandb
        accelerator.init_trackers(project_name="motion_pruning",
                                  config=args,
                                  init_kwargs={"wandb": {"name": "Finetuned_sd14_ours224_LCM_with_Tiny_Teacher"}},)
    ...
    teacher_pipe = AnimateDiffPipeline.from_pretrained(args.pretrained_teacher_model,motion_adapter=teacher_adapter,torch_dtype=weight_dtype)
    ...
    print(f' (3.3) create online unet ')
    test_adapter = MotionAdapter.from_pretrained("wangfuyun/AnimateLCM", torch_dtype=weight_dtype)
    test_pipe = AnimateDiffPipeline.from_pretrained(args.pretrained_teacher_model, motion_adapter=test_adapter,torch_dtype=weight_dtype)
    ...
    # teacher register
    teacher_motion_controller = MotionControl(is_teacher=True,
                                              train=True,
                                              frame_num=16,
                                              skip_layers=pruned_blocks, )
    ...
    trainable_params = []
    # check, all training and no zeros
    ...
    # copy the unet from the test_pipe

    #test_unet_config = test_unet.config
    #test_unet_state_dict = test_unet.state_dict()
    #target_unet = UNetMotionModel.from_config(test_unet_config)
    #target_unet.load_state_dict(test_unet_state_dict,strict=False)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(test_unet):
                ...
                print(f' input data to test unet')
                ...
                if args.loss_type == "l2":
                    # model pred should be same as target
                    # that mean this model should be consistency ...
                    loss = F.mse_loss(model_pred.float(),  # runed model pred value
                                      target.float(),  # target model value (learning the target unet)
                                      reduction="mean")
                ...
                for layer in teacher_layerwise_hidden_dict.keys():
                    ...
                    print(f' feature loss = {feature_loss} at {layer}' )

                # 11. Backpropagate on the online student model (`unet`)
                print(f' trining loss = {loss}')
            ...
            if accelerator.sync_gradients:
                ...
                update_ema(target_unet.parameters(),
                           test_unet.parameters(),
                           args.ema_decay)
