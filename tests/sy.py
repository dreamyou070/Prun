...
class SimpleAttention(nn.Module):
    ...
    def forward(self,
        ...
        query = query.view(-1, num_frames, self.heads, head_dim).transpose(1,2)  # [batch*pixel, head, num_frames, head_dim]
        key = key.view(-1, num_frames, self.heads, head_dim).transpose(1,2)  # [batch*pixel, head, num_frames, head_dim]
...
def set_simple_attention(unet, skip_layers):
    def register_editor(net, net_name):
        for name, subnet in net.named_children():
            ...
            if hasattr(net, 'children'):
                register_editor(subnet, final_name)
    register_editor(unet, "unet")
...
def main(args) :
    ...
    teacher_adapter = MotionAdapter.from_pretrained("wangfuyun/AnimateLCM", torch_dtype=torch.float16)
    ...
    def hook_fn_teacher(module, input, output):
        ...
        outputs_teacher.append(output[0].detach())  # Teacher 모델의 출력
    register_hook(teacher_unet, hook_fn_teacher)

    print(f' (3.2) student model')
    ...
    print(f' step 5. weight and device')
    ...
    student_unet, optimizer, lr_scheduler = accelerator.prepare(student_unet, optimizer, lr_scheduler)
    ...
    uncond_input_ids = tokenizer([""] * args.per_gpu_batch_size, return_tensors="pt", padding="max_length", max_length=77
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(student_unet):
                ...
                student_output = student_unet(noisy_model_input.to(dtype=student_unet.dtype),
                                              start_timesteps,
                                              encoder_hidden_states=prompt_embeds.to(dtype=student_unet.dtype)).sample
                ...
                if args.loss_type == "l2":
                    ...
                    distill_loss = torch.mean(
                        torch.sqrt((student_output.float() - teacher_output.float()) ** 2 + args.huber_c ** 2) - args.huber_c)
...
if __name__ == "__main__":
    ...
    # [2]
    parser.add_argument("--architecture", type=arg_as_list, default=[0,1,2,3,4,6,14,15,16,20])
