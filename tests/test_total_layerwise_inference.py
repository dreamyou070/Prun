...
layerwise_thredshold_dict = {'down_blocks.0.motion_modules.0' : 30, 'down_blocks.0.motion_modules.1' : 50 ,
                             'down_blocks.1.motion_modules.0' : 60}
                             #down_blocks.1.motion_modules.1' : 180,
                             #'down_blocks.2.motion_modules.0' : 200,'down_blocks.2.motion_modules.1' : 200,
                             #'down_blocks.3.motion_modules.0' : 200,'down_blocks.3.motion_modules.1' : 200,}
# if name in one of layerwise_thredshold_dict.keys() :
#     criteria = layerwise_thredshold_dict[name]
# code for [if name in one of layerwise_thredshold_dict.keys()]
...
def evaluation(prompt, evaluation_pipe, controller, save_folder,
    ...
    with torch.no_grad() :
        output = evaluation_pipe(prompt=prompt,
         ...
         #output = test_pipe(prompt="A space rocket with trails of smoke behind it launching into space from the desert, 4k, high resolution",
...
def main(args):
    ...
    out_dir = os.path.join(base_out_dir, args.sub_folder_name)
    ...
    original_state_dictionary = pruned_unet.state_dict()
    sparse_state_dictionary = {}
    #for name, value in original_state_dictionary.items() :
    ...
    base_folder = r'/scratch2/dreamyou070/Prun/result/20241017_layerwise_sorting_3'
    files = os.listdir(base_folder)
    # [1] --------------------------------------------------------------------------------------------------- #
    for name, module in pruned_unet.named_modules() :
        if 'motion' in name and module.__class__.__name__ == 'Linear' and 'ff.net.0.proj' not in name :
            # [1] get block name
            block_name = [k for k in layerwise_thredshold_dict.keys() if k in name]
            if len(block_name) > 0 :
                print(f'{name} is pruned')
                block_name = block_name[0]
                criteria = layerwise_thredshold_dict[block_name]
                ...
                if module.bias is not None :
                    print(f'sparse_bias = {sparse_bias}')
    ...
    for p, prompt in enumerate(custom_prompts):    
        if p < 51 : # 0,1,2
            evaluation(prompt, test_pipe, None, out_dir,
