...
class TransformerTemporalModel(ModelMixin, ConfigMixin):
    ...
    def add_positional_embeddings(self,
                                  num_frames=16):
        import math
        self.pe = torch.zeros(num_frames, self.in_channels, 1)  # [16,340,1]
        position = torch.arange(num_frames).unsqueeze(1)  # [16,1]
        div_term = torch.exp(torch.arange(0, self.in_channels, 2) * (-math.log(10000.0) / self.in_channels))  # [frame_num, dim]
        div_pos = position * div_term  # [16,1] * [170] = [16,170]
        self.pe[:, 0::2, 0] = torch.sin(div_pos)  # [16,1] * [170] = [16,170] # odd number = sin
        self.pe[:, 1::2, 0] = torch.cos(div_pos)  # [16,1] * [170] = [16,170] # odd number = sin
    ...
    def forward(
        ...
        # 1. Input
        batch_frames, channel, height, width = hidden_states.shape # what is channel
