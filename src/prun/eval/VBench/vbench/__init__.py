...
class VBench(object):
    ...
    def build_full_dimension_list(self, ):
        return ["subject_consistency", "background_consistency", "aesthetic_quality", "imaging_quality", "object_class",
                "multiple_objects", "color", "spatial_relationship", "scene", "temporal_style", 'overall_consistency', "human_action", "temporal_flickering",
    ...
    def build_full_info_json(self, videos_path, name, dimension_list, prompt_list=[], special_str='', verbose=False, mode='vbench_standard', **kwargs):
        ...
        if mode=='custom_input':
            ...
            if os.path.isfile(videos_path):
                cur_full_info_list = [{"prompt_en": get_prompt_from_filename(videos_path),
                                       "dimension": dimension_list,
                                       "video_list": [videos_path]}]
                if len(prompt_list) == 1:
                    cur_full_info_list[0]["prompt_en"] = prompt_list[0]
            else:
                print(f'start videos_path = {videos_path}')
    ...
    def evaluate(self,
                 ...
                 name,
                 prompt_list=[],
                 dimension_list=None, local=False, read_frame=False, mode='vbench_standard', **kwargs):

        results_dict = {}
        if dimension_list is None:
            dimension_list = self.build_full_dimension_list()
        # [1]
        submodules_dict = init_submodules(dimension_list, # [motion_dynamics, X, X]
                                          local=local,    # local
                                          read_frame=read_frame)
        # motion_model = submodules_dict['model']
        cur_full_info_path = self.build_full_info_json(videos_path, name, dimension_list, prompt_list, mode=mode, **kwargs)

        for dimension in dimension_list:
            print(f' dimension = {dimension}') # motion_dynamics
            dimension_module = importlib.import_module(f'vbench.{dimension}')
            # compute_background_consistency
            evaluate_func = getattr(dimension_module, f'compute_{dimension}') # compute_motion_dynamics
            """
            try:
                dimension_module = importlib.import_module(f'vbench.{dimension}')
            ...
            print(f' *** submodules_list = {submodules_list}')

            results = evaluate_func(cur_full_info_path,
                                    self.device,
                                    submodules_list,
                                    **kwargs)

            results_dict[dimension] = results # not tow but one
