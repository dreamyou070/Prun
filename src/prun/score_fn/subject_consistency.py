...
from prun.score_fn.utils import load_video, load_dimension_info, dino_transform, dino_transform_Image
import logging
...
def subject_consistency(model, video_list, device, read_frame):
    ...
    for video_path in video_list:
        video_sim = 0.0
        print(f' [subject consistency file] {video_path}')
        if read_frame:
...
def compute_subject_consistency(device,video_list, ):
    condition =  {'repo_or_dir': 'facebookresearch/dino:main',
                 'source': 'github',
                 'model': 'dino_vitb16',
                 'read_frame': None}
    dino_model = torch.hub.load(**condition).to(device)
    read_frame = None
