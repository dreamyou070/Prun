...
class AnimationPipeline(DiffusionPipeline):
    ...
    def decode_latents(self, latents, image_path):
        ...
        if image_path is not None:
            ...
            for i in range(video_length):
                target_img = video[0, :, i, :, :]  
                target_img = target_img.permute(1, 2, 0) 
                target_img = (target_img.numpy() * 255).astype(np.uint8)
                target_img = cv2.cvtColor(target_img, cv2.COLOR_RGB2BGR)
                processed_img = adjust_colors(src_img, target_img)
    ...
    def encode_img(self, image_path, height,  width):

        # resize and cropping # -----------------------------------------------------------
        if image_path is None:
            return None
        image = resize_and_crop(image_path, width, height)
        image = ToTensor()(image).to(self.device).unsqueeze(0)
        image = image * 2 - 1
        ...
        image = Image.fromarray((image[0].transpose(1,2,0)*255).astype(np.uint8))
        image.save("temp.png")
        return latent
    ...
    def __call__(
        ...
        **kwargs,
        ...
        do_classifier_free_guidance = (guidance_scale > 1.0) & do_classifier_free_guidance
        ...
        # ------------------------------------------------------------------------------------------------------------------------------ #
        # [1] start of imag encoding
        img_latent = self.encode_img(image_path,height,width)
        ...
        latents = self.prepare_latents(batch_size * num_videos_per_prompt, num_channels_latents, video_length,
                                       height, width, text_embeddings.dtype, device, generator, latents,)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                img_latent_input = torch.cat(
                    [img_latent] * 2) if (do_classifier_free_guidance and img_latent is not None) else img_latent


                # predict the noise residual
                noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embeddings,
                                       time_cond=w_embedding, img_latent = img_latent_input).sample.to(dtype=latents_dtype)
        ...
        # Post-processing
        video = self.decode_latents(latents,image_path)
